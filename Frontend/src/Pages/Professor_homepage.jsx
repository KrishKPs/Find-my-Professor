...
const Trial = () => {
  ...
  useEffect(() => {
    ...
    const fetchAppointments = async () => {
      ...
      setPendingCount(
        appointmentsData.filter((appointment) => appointment.status === 'Pending').length
      );
    };
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      if (response.data.message === 'Professor details updated successfully') {
        // Immediately update the professor state with the new data from the server response
        setProfessor({ ...professor, ...response.data.professor });

        // Exit edit mode after the state update
        setIsEditing(false);
  ...
  return (
    <div className="bg-gradient-to-r from-blue-800 to-teal-600 min-h-screen text-white p-8 flex flex-col items-center relative">
        ...
        <Card className="w-full md:w-3/4 lg:w-1/2 p-8 bg-white text-gray-900 rounded-3xl shadow-lg transform transition-all hover:scale-105 duration-300 mb-12">
          <div className="flex items-center mb-6 justify-between">
            ...
            <div className="flex-grow ml-6">
              {/* Professor name is not editable */}
      ...
      {showDropdown && (
        <div
          className={`fixed top-0 right-0 w-80 h-full bg-white text-black shadow-lg transform transition-all duration-300 ${
        ...
        {ApprovedMeetings.length > 0 && (
          <div>
            ...
            {ApprovedMeetings.map((appointment) => (
              <Card
                key={appointment._id}
                className="mb-4 p-4 bg-white text-gray-900 rounded-xl shadow-md"
              >
                ...
                <Button
                  onClick={() => handleCompleteAppointment(appointment._id, appointment.student_email)}
